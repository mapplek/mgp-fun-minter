...
export default function ContainerMint(props) {
    ...
    .then((resp) => {
        ...
        setMintedAmount(parseInt(resp[1].result))
        setPaused(resp[2].result)
    });

    useEffect(() => {
        if(!paused && mintedAmount <= 0) {
            ...
            {!confirmModalDisabled &&
                ...
                >
                    <div className={styles.modal_text_block}>
                        ...
                        <ul>
                            <li>当NFTは転送・販売が不可能なSBT (Soul Bound Token) 形式となります。</li>
                            <li>記載した入力内容にてトランザクションを通した後は、再度名前を変更することはできません。</li>
